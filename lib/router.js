...
var attachPageViewTracking = function attachPageViewTracking(options) {
    ...
    if(typeof Nearest.googleAnalytics !== 'undefined' ) {
      _.each(Nearest.googleAnalytics.getOptions(),function(ga){
        window.ga && window.ga(ga.create.name+'.send', 'pageview');
      });
...
var attachPageSetter = function attachPageSetter(options) {
    ...
    if(typeof Nearest.googleAnalytics !== 'undefined' ) {
      _.each(Nearest.googleAnalytics.getOptions(),function(ga){
        console.log('create',ga.create.name);
        options.onRun = function onRun() {
          window.ga && window.ga(ga.create.name+'.set', 'page', this.url);
        };
      });
      return callEventHandlerOrNext.call(this, originalOnRun, arguments);
    }
